...
const SmartRecommendations: React.FC<{ 
  ...
  // Get avoidance methods for this emotion (removed UI toggle)
...
const [predictions, setPredictions] = useState<Record<string, number> | null>(null);
  ...
  // Known emotions (key = model key, label = user-facing label)
  ...
  // Build emotionResults from last backend predictions when available.
  const emotionResults = useMemo(() => {
    return knownEmotions.map(e => {
      const prob = predictions && typeof predictions[e.key] === 'number'
        ? Math.round(predictions[e.key] * 100)
        : (e.key === currentEmotion ? confidence : 0);
  ...
  }, [knownEmotions, predictions, currentEmotion, confidence]);
...
const handleImageUpload = useCallback(async (imageBlob: Blob) => {
  ...
  try {
    ...
    // Save full predictions so UI can show a consistent breakdown
    if (result.predictions && typeof result.predictions === 'object') {
      setPredictions(result.predictions);
    }
    setConfidence(Math.floor((result.predictions?.[result.top_emotion] ?? 0) * 100));
  } catch (err) {
    console.warn(err);
...
useEffect(() => {
  ...
  if (isCameraActive && videoRef.current) {
    interval = setInterval(() => {
      void captureAndSendFrame(); // Sends image to backend
    }, 5000);
  }
...
}, [isCameraActive, handleImageUpload]);
    ...
    <div className={`w-screen h-screen bg-gradient-to-br ${theme.bg} overflow-hidden transition-all duration-1000`}>
      ...
      <div className={`h-16 ${theme.navBg} backdrop-blur-xl border-b ${theme.border} flex items-center justify-between px-6 shadow-sm`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center cursor-pointer hover:scale-105 transition-transform shadow-md ${profile?.photoDataUrl ? 'overflow-hidden' : 'bg-gradient-to-br ' + theme.accent}`}> 
    ...
    <div className="text-center text-slate-500">
      ...
      <p className="text-sm"
       style={{ fontFamily: 'var(--font-space-grotesk), Space Grotesk, sans-serif' }}>Click &quot;Start Camera&quot; to begin emotion detection</p>
              ...
              {`w-full h-64 bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl border ${theme.border} flex items-center justify-center overflow-hidden`}
              {/* Emotion Result */}
                        ...
                        {emotionResults.map((emotion, index) => (
                          <div 
                            ...
                            className={`flex items-center justify-between p-2 rounded-lg ${
                              emotion.key === currentEmotion 
                                ? 'bg-white/80 font-semibold' 
                                : 'bg-white/40'
                            }`}
                          >
                            <span>{emotion.key}</span>
                            <span className={emotion.key === currentEmotion ? theme.textSecondary : 'text-slate-500'}>
                              {emotion.key === currentEmotion ? emotion.confidence : Math.floor(Math.random() * 20) + 5}%
                            </span>
                          </div>
                        ))}
                      </div>
